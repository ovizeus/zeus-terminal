...
export function _indRenderHook(): void {
  // [2026-06-26] Defensive isolation — each indicator render runs in its own
  // try/catch so a single throwing indicator can no longer blank the WHOLE chart
  // on a symbol switch (the old bug: one bad indicator emptied everything,
  // "Christmas-lights-in-series"). One failure now stays isolated; the rest render.
  ...
  if (w.S.activeInds.ichimoku) safe(updateIchimoku)
  if (w.S.activeInds.fib) safe(updateFib)
  if (w.S.activeInds.pivot) safe(updatePivot)
  if (w.S.activeInds.vp) safe(updateVP)
  if (w.S.activeInds.magnes) safe(updateMagnes)
  if (w.S.activeInds.rsi14 && w._rsiInited) safe(updateRSI)
  if (w.S.activeInds.stoch && w._stochInited) safe(updateStoch)
  if (w.S.activeInds.atr && w._atrInited) safe(updateATRInd)
  if (w.S.activeInds.obv && w._obvInited) safe(updateOBV)
  if (w.S.activeInds.mfi && w._mfiInited) safe(updateMFI)
  if (w.S.activeInds.cci && w._cciInited) safe(updateCCI)
  // [2026-06-16] batch-1 overlays
  if (w.S.activeInds.sma) safe(updateSMA)
  if (w.S.activeInds.hma) safe(updateHMA)
  if (w.S.activeInds.psar) safe(updatePSAR)
  if (w.S.activeInds.kc) safe(updateKC)
  if (w.S.activeInds.dc) safe(updateDC)
  // [2026-06-16] batch-2 oscillators
  if (w.S.activeInds.adx && w._adxInited) safe(updateADX)
  if (w.S.activeInds.willr && w._willrInited) safe(updateWILLR)
  if (w.S.activeInds.roc && w._rocInited) safe(updateROC)
  if (w.S.activeInds.cmf && w._cmfInited) safe(updateCMF)
  if (w.S.activeInds.ao && w._aoInited) safe(updateAO)
  // [2026-06-16] batch-3
  if (w.S.activeInds.vwma) safe(updateVWMA)
  if (w.S.activeInds.aroon && w._aroonInited) safe(updateAroon)
  if (w.S.activeInds.trix && w._trixInited) safe(updateTrix)
  if (w.S.activeInds.uo && w._uoInited) safe(updateUO)
  if (w.S.activeInds.chop && w._chopInited) safe(updateChop)
  if (w.S.activeInds.kera) safe(updateKera)
  if (w.S.activeInds.aether) safe(updateAether)
  if (w.S.activeInds.ms) safe(updateMS)
  if (w.S.activeInds.nem) safe(updateNem)
  if (w.S.activeInds.iris) safe(updateIris)
  if (w.S.activeInds.pythia) safe(updatePythia)
  if (w.S.activeInds.plutus) safe(updatePlutus)
  if (w.S.activeInds.helios && w._heliosInited) safe(updateHelios)
  if (w.S.activeInds.hyperion && w._hyperionInited) safe(updateHyperion)
  if (w.S.activeInds.eunomia && w._eunomiaInited) safe(updateEunomia)
  if (w.S.activeInds.kronos && w._kronosInited) safe(updateKronos)
  if (w.S.activeInds.hermes) safe(updateHermes)
  if (w.S.activeInds.charon) safe(updateCharon)
  if (w.S.activeInds.atlas && w._atlasInited) safe(updateAtlas)
  if (w.S.activeInds.eos) safe(updateEos)
  if (w.S.activeInds.pantheon && w._pantheonInited) safe(updatePantheon)
  if (w.S.activeInds.aegis) safe(updateAegis)
  if (w.S.activeInds.selene && w._seleneInited) safe(updateSelene)
  if (w.S.activeInds.kratos) safe(updateKratos)
  if (w.S.activeInds.prometheus) safe(updatePrometheus)
  if (w.S.activeInds.mnemosyne) safe(updateMnemo)
  if (w.S.activeInds.themis && w._themisInited) safe(updateThemis)
  if (w.S.activeInds.erebus && w._erebusInited) safe(updateErebus)
  if (w.S.activeInds.anemoi && w._anemoiInited) safe(updateAnemoi)
  if (w.S.activeInds.cerberus && w._cerberusInited) safe(updateCerberus)
  if (w.S.activeInds.proteus && w._proteusInited) safe(updateProteus)
  if (w.S.activeInds.typhon && w._typhonInited) safe(updateTyphon)
  if (w.S.activeInds.styx && w._styxInited) safe(updateStyx)
  if (w.S.activeInds.geras && w._gerasInited) safe(updateGeras)
  if (w.S.activeInds.ouranos) safe(updateOuranos)
  if (w.S.activeInds.hades) safe(updateHades)
  if (w.S.activeInds.athena) safe(updateAthena)
  if (w.S.activeInds.echo) safe(updateEcho)
  if (w.S.activeInds.kairos && w._kairosInited) safe(updateKairos)
  if (w.S.activeInds.tyche) safe(updateTyche)
  if (w.S.activeInds.nyx && w._nyxInited) safe(updateNyx)
  if (w.S.activeInds.olympus) safe(updateOlympus)
  if (w.S.activeInds.dolos) safe(updateDolos)
  if (w.S.activeInds.gaia) safe(updateGaia)
  if (w.S.activeInds.ananke) safe(updateAnanke)
  if (w.S.activeInds.psyche && w._psycheInited) safe(updatePsyche)
  if (w.S.activeInds.hubris) safe(updateHubris)
  if (w.S.activeInds.okeanos) safe(updateOkeanos)
  if (w.S.activeInds.aurora) safe(updateAurora)
  if (w.S.activeInds.argus) safe(updateArgus)
  if (w.S.activeInds.orion) safe(updateOrion)
  if (w.S.activeInds.phoenix) safe(updatePhoenix)
  if (w.S.activeInds.nephele) safe(updateNephele)
  if (w.S.activeInds.morpheus) safe(updateMorpheus)
  if (w.S.activeInds.metis) safe(updateMetis)
  if (w.S.activeInds.harmonia) safe(updateHarmonia)
  if (w.S.activeInds.daimon) safe(updateDaimon)
  if (w.S.activeInds.boreas) safe(updateBoreas)
  if (w.S.activeInds.mentor) safe(updateMentor)
  if (w.S.activeInds.apollo) safe(updateApollo)
}
