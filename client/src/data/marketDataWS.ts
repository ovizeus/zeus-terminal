...
export function toggleSnd(): void {
  ...
  const e = w.el('snd'); if (e) e.innerHTML = w.S.soundOn ? w._ZI.bell : w._ZI.bellX
}
...
export function applyChartColors(): void { const uc = w.el('ccBull')?.value || '#00d97a'; const dc = w.el('ccBear')?.value || '#ff3355'; const uw = w.el('ccBullW')?.value || '#00d97a77'; const dw = w.el('ccBearW')?.value || '#ff335577'; if (w.cSeries) w.cSeries.applyOptions({ upColor: uc, downColor: dc, borderUpColor: uc, borderDownColor: dc, wickUpColor: uw, wickDownColor: dw }); w.toast('Colors applied'); if (typeof w._usScheduleSave === 'function') w._usScheduleSave() }
export function setCandleStyle(style: string, btn: any): void { document.querySelectorAll('#ct-candles .qb').forEach((b: any) => b.classList.remove('act')); if (btn) btn.classList.add('act'); w.toast('Style: ' + style) }
export function setTZ(tz: string, btn: any): void { w.S.tz = tz; document.querySelectorAll('#cst .qb').forEach((b: any) => b.classList.remove('act')); if (btn) btn.classList.add('act'); const n: any = { 'Europe/Bucharest': 'RO', 'UTC': 'UTC', 'America/New_York': 'NY', 'Asia/Tokyo': 'TK', 'Europe/London': 'LN' }; const lbl = w.el('chartTZLbl'); if (lbl) lbl.textContent = n[tz] || tz; w.toast('Timezone: ' + tz); if (typeof w._usScheduleSave === 'function') w._usScheduleSave() }
export function applyHeatmapSettings(): void { const hs = w.S.heatmapSettings; const gv = (id: string) => +(w.el(id)?.value) || 0; hs.lookback = gv('hmLookback') || 400; hs.pivotWidth = gv('hmPivotW') || 1; hs.atrLen = gv('hmAtrLen') || 121; hs.atrBandPct = gv('hmAtrBand') || 0.05; hs.extendUnhit = gv('hmExtend') || 30; hs.heatContrast = gv('hmContrast') || 0.3; hs.minWeight = 0; hs.keepTouched = w.el('hmKeepTouched')?.checked !== false; hs.longCol = w.el('hmLongCol')?.value || '#01c4fe'; hs.shortCol = w.el('hmShortCol')?.value || '#ffe400'; if (w.S.overlays.liq) w.renderHeatmapOverlay(); closeM('mcharts'); w.toast('Heatmap updated'); if (typeof w._usScheduleSave === 'function') w._usScheduleSave() }
...
export function applySessionSettings(): void { if (typeof w._usScheduleSave === 'function') w._usScheduleSave(); w.toast('Session settings saved') }

// ===== SUPREMUS + ZS =====
export function applyZS(): void { w.S.zsSettings = w.S.zsSettings || {}; const cbIds = ['zshh', 'zshl', 'zsll', 'zslh', 'zsbb', 'zsfi', 'zspi', 'zsvi', 'zsse', 'zsds', 'zspu', 'zspd', 'zspivot', 'zsvwap', 'zsShowZones', 'zsExtendZones']; cbIds.forEach((id: string) => { const e = w.el(id); if (e) w.S.zsSettings[id] = e.checked }); const colIds = ['zshhCol', 'zshlCol', 'zslhCol', 'zsllCol', 'zsUpperCol', 'zsLowerCol', 'zsVwapDc', 'zsVwapWc', 'zsVwapMc']; colIds.forEach((id: string) => { const e = w.el(id); if (e) w.S.zsSettings[id] = e.value }); const numIds = ['zsZoneWidth', 'zsPivotLen', 'zsPivotCount']; numIds.forEach((id: string) => { const e = w.el(id); if (e) w.S.zsSettings[id] = +e.value }); ['zsVwapD', 'zsVwapW', 'zsVwapM'].forEach((id: string) => { const e = w.el(id); if (e) w.S.zsSettings[id] = e.checked }); w.toast('Supremus settings saved', 3000, w._ZI.crown); if (w.S.overlays.zs) { clearZS(); renderZS() } }
