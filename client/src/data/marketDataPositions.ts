...
export function renderPendingOrders(): void {
  ...
  cont.innerHTML = allPending.map(function (ord: any) {
    ...
    const curPrice = getSymPrice(ord) || (w.allPrices[ord.sym] || 0)
    const distPct = curPrice > 0 ? (((ord.limitPrice - curPrice) / curPrice) * 100).toFixed(2) : '?'
    return '<div class="pos-row pos-pending" style="border-color:' + sideColor + '"><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-weight:700;color:' + sideColor + '"><span style="background:#00d4ff22;color:#00d4ff;padding:1px 6px;border-radius:3px;font-size:9px;font-weight:700;margin-right:4px"> WAITING LIMIT</span>' + escHtml(ord.side) + ' ' + symBase + ' ' + ord.lev + 'x' + modeBadge + '</span><div style="display:flex;gap:4px"><button onclick="modifyPendingPrice(\'' + ord.id + '\')" style="padding:6px 10px;background:#001a33;border:1px solid #00aaff;color:#00d4ff;border-radius:3px;font-size:9px;cursor:pointer;font-weight:700;min-height:36px">EDIT MODIFY</button><button onclick="cancelPendingOrder(\'' + ord.id + '\')" style="padding:6px 10px;background:#2a0010;border:1px solid #ff4466;color:#ff4466;border-radius:3px;font-size:9px;cursor:pointer;font-weight:700;min-height:36px">\u2715 CANCEL</button></div></div><div style="display:flex;justify-content:space-between;font-size:12px;margin-top:3px;color:var(--dim)"><span>Limit: $' + w.fP(ord.limitPrice) + ' | Size: $' + fmt(ord.size) + '</span><span>Now: $' + (curPrice > 0 ? w.fP(curPrice) : '\u2014') + ' (' + distPct + '%)</span></div><div style="font-size:11px;color:var(--dim);margin-top:1px">' + (ord.sl ? 'SL: $' + w.fP(ord.sl) + ' ' : '') + (ord.tp ? 'TP: $' + w.fP(ord.tp) + ' ' : '') + '| ' + ageStr + ' ago' + (ord.exchangeOrderId ? ' | OID: ' + ord.exchangeOrderId : '') + '</div></div>'
  }).join('')
}
...
export function savePosSLTP(posId: any, mode: string): void {
  ...
  if (mode === 'demo') {
    ...
    Promise.all(promises).then(function () { w.renderLivePositions(); w.toast('LIVE SL/TP updated') }).catch(function (err: any) { w.toast('SL/TP update failed: ' + (err.message || err)); w.renderLivePositions() })
  }
}
...
export function renderLivePositions(): void {
  ...
  const html = live.map(function (pos: any) {
    ...
    return `<div class="pos-row ${pos.side === 'LONG' ? 'pos-long' : 'pos-short'}"><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-weight:700">${w._ZI.dRed} ${escHtml(pos.side)} ${symBase} ${pos.lev}x</span><button data-live-id="${pos.id}" style="padding:10px 14px;background:#2a0010;border:2px solid #ff4466;color:#ff4466;border-radius:4px;font-size:10px;cursor:pointer;min-height:52px;font-weight:700">\u2715 CLOSE</button></div><div style="display:flex;justify-content:space-between;font-size:13px;margin-top:3px"><span style="color:var(--dim)">Entry: $${w.fP(pos.entry)} | Now: $${w.fP(cur)}</span><span style="color:${pnl >= 0 ? 'var(--grn)' : 'var(--red)'}">${pnl >= 0 ? '+' : ''}$${pnl.toFixed(2)} (${pnlPct}%)</span></div>${pos.liqPrice ? `<div style="font-size:12px;color:#ff3355;margin-top:1px">LIQ: $${w.fP(pos.liqPrice)}</div>` : ''}<div style="display:flex;gap:4px;margin-top:3px;align-items:center"><span style="font-size:10px;color:#ff6644;width:22px">SL:</span><input id="slEdit_${pos.id}" type="number" step="0.1" value="${pos.sl || ''}" placeholder="\u2014" style="flex:1;background:#0a0a14;border:1px solid #333;color:#ff6644;padding:3px 5px;border-radius:3px;font-size:11px;font-family:var(--ff);width:60px"><span style="font-size:10px;color:#00ff88;width:22px">TP:</span><input id="tpEdit_${pos.id}" type="number" step="0.1" value="${pos.tp || ''}" placeholder="\u2014" style="flex:1;background:#0a0a14;border:1px solid #333;color:#00ff88;padding:3px 5px;border-radius:3px;font-size:11px;font-family:var(--ff);width:60px"><button onclick="savePosSLTP('${pos.id}','live')" style="padding:3px 8px;background:#001a22;border:1px solid #00aaff;color:#00d4ff;border-radius:3px;font-size:9px;cursor:pointer;font-weight:700;min-height:24px">SAVE</button></div></div>`
  }).join('')
...
export function closeLivePos(id: any, reason?: string): void {
  ...
  if (typeof w.liveApiClosePosition === 'function') {
    w.liveApiClosePosition(pos).then(function (res: any) {
      ...
      w.atLog('warn', 'LIVE CLOSE FAILED: ' + (err.message || err)); w.toast('Close failed: ' + (err.message || 'still open on exchange'))
      renderLivePositions()
