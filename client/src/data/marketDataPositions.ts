...
export function savePosSLTP(posId: any, mode: string): void {
  ...
  if (mode === 'demo') {
    ...
    Promise.allSettled(promises).then(function (results: any[]) { var failed = results.filter(function (r: any) { return r.status === 'rejected' }); if (failed.length === 0) { w.toast('LIVE SL/TP updated') } else { w.toast('Partial update — ' + failed.length + ' failed, verifică exchange') }; w.renderLivePositions() })
  }
}
...
export function renderDemoPositions(): void {
  ...
  if (!manualPos.length) { table.innerHTML = '<div style="color:var(--dim);text-align:center;padding:8px">No open positions</div>' }
    ...
    const html = manualPos.map((pos: any) => {
      ...
      if (!curPrice || !Number.isFinite(curPrice) || curPrice <= 0) { pos.pnl = 0; const symBase = w.escHtml((pos.sym || 'BTC').replace('USDT', '')); return `<div class="pos-row"><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-weight:700">${w.escHtml(pos.side)} ${symBase} ${pos.lev}x</span><button data-id="${pos.id}" style="padding:10px 14px;background:#2a0010;border:2px solid #ff4466;color:#ff4466;border-radius:4px;font-size:10px;cursor:pointer;min-height:52px;font-weight:700">\u2715 CLOSE</button></div><div style="font-size:13px;margin-top:3px;color:#ff8800">Price unavailable</div></div>` }
