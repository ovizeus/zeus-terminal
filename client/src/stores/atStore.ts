...
type ATStore = ATState & {
  ...
  /** [Phase 3B] Reset all state on logout. Defaults to demo mode, disabled, no stats. */
  reset: () => void
}

const _DEFAULT_AT_STATE = {
  enabled: false,
  mode: 'demo' as const,
  running: false,
  killTriggered: false,
...
export const useATStore = create<ATStore>()((set) => ({
  ...
  // [Phase 3B] Logout reset — wipes AT state, stats, config, UI to defaults.
  reset: () => set({
    ..._DEFAULT_AT_STATE,
    config: { ...DEFAULT_AT_CONFIG },
    ui: { ...DEFAULT_AT_UI },
