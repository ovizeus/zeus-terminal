...
async function scanLiquidityMagnets() {
  if (!S.price) return;
  const now = Date.now();
  ...
  if (ob.asks.length && ob.bids.length) {
    const allQ = [...ob.asks.map(x => x.q), ...ob.bids.map(x => x.q)];
  ...
  const clusters = Object.values(S.btcClusters || {});
  if (clusters.length) {
    const maxVol = Math.max(...clusters.map(c => c.vol), 1);
    clusters.sort((a, b) => b.vol - a.vol).slice(0, 8).forEach(c => {
  ...
  if (S.klines && S.klines.length > 50) {
    ...
    const maxVol = Math.max(...recentBars.map(k => k.volume));
    // Find bars with volume > 70th percentile
    const volThreshold = maxVol * 0.7;
    recentBars.forEach(k => {
  ...
  if (S.atr && S.price) {
    const atr = S.atr;
    [1, 2, 3, 1.618, 2.618].forEach(mult => {
  ...
  sorted.forEach(m => {
    const existing = merged.find(e => Math.abs(e.price - m.price) < priceZone && e.direction === m.direction);
    if (existing) {
  ...
  // ─── 6. SORT & FILTER ─────────────────────────────────────────
  const above = merged.filter(m => m.direction === 'above' && m.price > S.price && m.strength > 20)
    .sort((a, b) => a.price - b.price) // nearest first
    .slice(0, 5);
  const below = merged.filter(m => m.direction === 'below' && m.price < S.price && m.strength > 20)
  ...
  updateMagnetBias();
  // Recalc BM.liqCycle distances + sweep imediat după scan nou
...
function renderMagnets() {
  ...
  // Render lists
  const renderList = (list, containerId, isAbove) => {
...
async function runBacktest() {
  ...
  BT_INDICATORS.forEach(ind => { results[ind.id] = { wins: 0, losses: 0, trades: [], pnls: [] }; });
  ...
  // RSI per bar
  function calcRSIArr(prices, p = 14) {
  ...
  // [FIX v85 BUG6] Verificări de lungime înainte de slice/concat
  ...
  // Necesitem cel puțin 25+9=34 bare pentru signal MACD
  let macdFull;
  if (macdLine.length < 34) {
    ...
    // Asigură aceeași lungime cu macdLine
  ...
  function buildAtrArray(brs, period) {
...
function renderBacktestResults(results, equityCurve, fwdBars, lookback, minMovePct) {
  const rows = [];
  let totalWins = 0, totalTrades = 0, bestWR = 0, bestName = '—';
  let confWR = 0;

  BT_INDICATORS.forEach(ind => {
    const r = results[ind.id];
    const tot = r.wins + r.losses;
    if (!tot) return;
    const wr = Math.round(r.wins / tot * 100);
    const avgWin = r.pnls.filter(p => p > 0).reduce((a, b) => a + b, 0) / (r.pnls.filter(p => p > 0).length || 1);
    const avgLoss = Math.abs(r.pnls.filter(p => p < 0).reduce((a, b) => a + b, 0) / (r.pnls.filter(p => p < 0).length || 1));
  ...
  if (svgEl && equityCurve.length > 2) {
    ...
    const w = 400, h = 50;
    const pathPts = pts.map((v, i) => `${(i / (pts.length - 1)) * w},${h - ((v - minV) / range * h * 0.85 + h * 0.07)}`).join(' ');
...
function calcVWAPBands(klines) {
  ...
  const dayBars = klines.filter(k => k.time >= dayStart);
  if (dayBars.length < 2) return null;
  let cumTPV = 0, cumV = 0;
  const vwapData = []; const upper1 = []; const lower1 = []; const upper2 = []; const lower2 = [];
  let cumVar = 0;
  dayBars.forEach((k, i) => {
...
function renderVWAP() {
  // Clear existing
  vwapSeries.forEach(s => { try { mainChart.removeSeries(s); } catch (_) { } });
  vwapSeries = [];
  if (!S.vwapOn || !S.klines.length) return;
  const res = calcVWAPBands(S.klines);
  if (!res) return;
  try {
    const vw = mainChart.addLineSeries({ color: '#00d97a', lineWidth: 2, priceLineVisible: false, lastValueVisible: true, title: 'VWAP' });
    vw.setData(res.vwap); vwapSeries.push(vw);
    const u1 = mainChart.addLineSeries({ color: '#00d97a66', lineWidth: 1, priceLineVisible: false, lastValueVisible: false, lineStyle: 2 });
    u1.setData(res.upper1); vwapSeries.push(u1);
    const l1 = mainChart.addLineSeries({ color: '#00d97a66', lineWidth: 1, priceLineVisible: false, lastValueVisible: false, lineStyle: 2 });
    l1.setData(res.lower1); vwapSeries.push(l1);
    const u2 = mainChart.addLineSeries({ color: '#00d97a33', lineWidth: 1, priceLineVisible: false, lastValueVisible: false, lineStyle: 3 });
    u2.setData(res.upper2); vwapSeries.push(u2);
    const l2 = mainChart.addLineSeries({ color: '#00d97a33', lineWidth: 1, priceLineVisible: false, lastValueVisible: false, lineStyle: 3 });
...
function toggleVWAP(btn) {
  ...
  else { vwapSeries.forEach(s => { try { mainChart.removeSeries(s); } catch (_) { } }); vwapSeries = []; }
...
function oviReadSettings() {
  ...
  const wm = document.querySelector('input[name="oviWeightMode"]:checked');
  c.weightMode = wm ? wm.value : 'Vol x Range';
}

function oviApplySettings() {
  oviReadSettings();
  if (S.oviOn) renderOviLiquid();
}

// ── ATR calculation ───────────────────────────────────────────
function oviCalcATR(klines, period) {
...
function oviPivots(klines, width, side) {
  // side: 'high' | 'low'
...
// ── Color from weight ─────────────────────────────────────────
function oviColor(weight, side, vMin, vMax, contrast, longCol, shortCol, alpha) {
...
function oviCalcPockets(klines) {
  const c = S.oviCfg;
  const n = klines.length;
  ...
  const weights = klines.map(k => oviWeightAt(k, c.weightMode));
  const wArr = weights.filter(w => w > 0);
  ...
  function addPocket(idx, side, price, weight) {
  ...
  return pockets.filter(p => (n - 1 - p.idx) <= c.lookback);
}

// ── Render OVI LIQUID on chart ────────────────────────────────
function renderOviLiquid() {
  clearOviLiquid();
  ...
  const allW = pockets.map(p => p.weight);
  ...
  pockets.forEach(p => {
    ...
    try {
      // TOP border line
      const topS = mainChart.addLineSeries({
        color: lineCol,
        lineWidth: 1,
      ...
      topS.setData(slice.map(k => ({ time: k.time, value: p.top })));
      oviSeries.push(topS);

      // BOTTOM border line
      const botS = mainChart.addLineSeries({
        color: lineCol,
        lineWidth: 1,
        priceLineVisible: false,
        lastValueVisible: false,
      ...
      botS.setData(slice.map(k => ({ time: k.time, value: p.bot })));
      oviSeries.push(botS);
      ...
      const midS = mainChart.addLineSeries({
        color: fillCol,
        lineWidth: lineW,
        priceLineVisible: false,
      ...
      midS.setData(slice.map(k => ({ time: k.time, value: mid })));
      oviSeries.push(midS);
...
function oviRenderScale(gMin, gMax) {
  // FIX 14: showScale actually creates visible price-line legend on chart
  const fmtW = w => w >= 1e9 ? (w / 1e9).toFixed(1) + 'B' : w >= 1e6 ? (w / 1e6).toFixed(1) + 'M' : w >= 1e3 ? (w / 1e3).toFixed(0) + 'K' : w.toFixed(0);
  try {
    const kl = S.klines; if (!kl.length) return;
    const last = kl[kl.length - 1];
    const c = S.oviCfg;
    const scaleS = mainChart.addLineSeries({
    ...
    oviSeries.push(scaleS);
  } catch (_) { }
}

// ── Clear all OVI series ──────────────────────────────────────
function clearOviLiquid() {
  oviSeries.forEach(s => { try { mainChart.removeSeries(s); } catch (_) { } });
  oviSeries = [];
}

// ── Toggle OVI LIQUID on/off ──────────────────────────────────
function toggleOviLiquid(btn) {
  S.oviOn = !S.oviOn;
  if (btn) {
...
function renderPnlLab() {
  ...
  try {
    ...
    var profFast = typeof calcExpectancyByProfile === 'function' ? calcExpectancyByProfile('fast') : { expectancy: 0, trades: 0, wr: 0 };
    var profSwing = typeof calcExpectancyByProfile === 'function' ? calcExpectancyByProfile('swing') : { expectancy: 0, trades: 0, wr: 0 };
    var profDef = typeof calcExpectancyByProfile === 'function' ? calcExpectancyByProfile('defensive') : { expectancy: 0, trades: 0, wr: 0 };
    ...
    if (weekly.length > 0) {
      ...
      weekly.forEach(function (w, i) {
        ...
        var col = w.netPnl >= 0 ? 'grn' : 'red';
        html += '<div class="pnl-lab-card"><div class="pnl-lab-card-label">' + label + '</div>';
        html += '<div class="pnl-lab-card-val" style="color:var(--' + col + ')">$' + w.netPnl.toFixed(2) + '</div>';
        html += '<div class="pnl-lab-card-sub">' + w.trades + 't \u00b7 ' + w.wins + 'W/' + w.losses + 'L</div></div>';
      });
      html += '</div></div>';
    }
...
function _pnlLabCard(label, value, colorKey) {
...
function _pnlLabProfileCard(name, data) {
...
window.togglePnlLab = togglePnlLab;
window.renderPnlLab = renderPnlLab;

// ── Long-press / right-click on OVI button → open settings ───
(function () {
  let _oviHold;
  const btn = () => el('oviBtn');
  const openSettings = () => { el('oviPanel').style.display = 'block'; };
...
// Session overlays
const _origRenderChart_ovi = window.renderChart;
if (typeof renderChart === 'function') {
  // Will be patched after renderChart is defined — see below
}
...
function toggleSession(sess, btn) {
...
// [FIX v85 BUG10] Funcție de curățare completă a tuturor seriilor de sesiune (apelată la schimb simbol)
function clearAllSessionOverlays() {
  Object.keys(sessionSeries).forEach(sess => {
    if (sessionSeries[sess] && sessionSeries[sess].length) {
      sessionSeries[sess].forEach(s => {
        try { if (mainChart) mainChart.removeSeries(s); } catch (_) { }
      });
      sessionSeries[sess] = [];
    }
  });
...
function renderSessionOverlay(sess, on) {
  if (!mainChart) return; // [FIX v85 BUG10] Guard: chart poate să nu existe
  // Remove existing
  if (sessionSeries[sess]) {
    sessionSeries[sess].forEach(s => { try { mainChart.removeSeries(s); } catch (_) { } });
    sessionSeries[sess] = [];
  }
  ...
  blocks.forEach(bl => {
    try {
      const startLine = mainChart.addLineSeries({ color: cfg.borderColor, lineWidth: 1, priceLineVisible: false, lastValueVisible: false, lineStyle: 1 });
      // Draw vertical-ish indicator at session boundaries
      const blBars = S.klines.filter(k => k.time >= bl.start && k.time <= bl.end);
      if (blBars.length < 1) return;
      const maxP = Math.max(...blBars.map(k => k.high));
      const minP = Math.min(...blBars.map(k => k.low));
      startLine.setData(blBars.map(k => ({ time: k.time, value: (maxP + minP) / 2 })));
