/* ───────────────────────────────────────────────────────────────
   ...
   TradingView-style interactive drawing.
   H-lines: native LWC createPriceLine (follows chart perfectly)
   Trendlines: native LWC addLineSeries (follows chart perfectly)
   Both use draggable HTML handle overlays.
   ─────────────────────────────────────────────────────────────── */
(function () {
  'use strict';
  if (window.__ZEUS_DRAW__) return;
  window.__ZEUS_DRAW__ = true;
  ...
  var _visible = true;
  var _dragging = null;
  var _tlineStep = 0, _tlinePendingP1 = null;
  var _selectedId = null;
  var _handleContainer = null;
  ...
  function _chart() { return window._zMainChart || null; }
  function _series() { return window._zCSeries || null; }
  ...
  function _toast(msg) {
    ...
    clearTimeout(window._dtToastTimer);
    window._dtToastTimer = setTimeout(function () { t.style.opacity = '0'; }, 1200);
  }
  ...
  function _timeAtX(clientX) {
    ...
    try {
      ...
      if (log != null && typeof S !== 'undefined' && S.klines) {
        var idx = Math.round(log);
        if (idx >= 0 && idx < S.klines.length) return S.klines[idx].time;
      }
    } catch(_) {}
    return null;
  }
  function _priceToY(price) { try { return _series().priceToCoordinate(price); } catch(_) { return null; } }
  function _timeToX(time) { try { return _chart().timeScale().timeToCoordinate(time); } catch(_) { return null; } }

  // ── Persistence ──
  function _save() {
    try { localStorage.setItem(LS_KEY, JSON.stringify({
      lines: _lines.map(function(l) {
        var o = { id:l.id, type:l.type, color:l.color };
  ...
  function _createHandle(lineId, idx, color) {
    ...
    el.addEventListener('mousedown', function(e) { _startDrag(e, lineId, idx); e.preventDefault(); e.stopPropagation(); });
  ...
  function _createDeleteBtn(lineId) {
    var el = document.createElement('div');
    el.className = 'dt-del';
    el.innerHTML = '✕';
    ...
    el.addEventListener('click', function(e) { _removeLine(lineId); e.stopPropagation(); });
    el.addEventListener('touchend', function(e) { _removeLine(lineId); e.preventDefault(); e.stopPropagation(); }, { passive: false });
    return el;
  }

  // ── Add H-Line (native LWC price line — follows chart automatically) ──
  function _addHLine(price, color, existingId) {
  ...
  function _addTLine(p1, p2, color, existingId) {
  ...
  function _updateTLineSeries(line) {
  ...
  function _removeLine(id) {
    for (var i = 0; i < _lines.length; i++) {
      if (_lines[i].id === id) {
        ...
        l.handles.forEach(function(h) { if (h.parentElement) h.parentElement.removeChild(h); });
  ...
  // ── Select / Deselect ──
  function _selectLine(id) {
    _selectedId = id;
    _lines.forEach(function(l) {
      l.selected = (l.id === id);
      l.handles.forEach(function(h) { h.style.display = l.selected ? 'block' : 'none'; });
  ...
  // ── Drag ──
  function _startDrag(e, lineId, handleIdx) {
    _dragging = { lineId:lineId, idx:handleIdx, _touch:!!e._touch };
    _selectLine(lineId);
  }
  function _onDragMove(clientX, clientY) {
    if (!_dragging) return;
    var line = _lines.find(function(l) { return l.id === _dragging.lineId; });
  ...
  document.addEventListener('mousemove', function(e) { if (_dragging && !_dragging._touch) _onDragMove(e.clientX, e.clientY); });
  ...
  // ── Chart interaction ──
  function _isOnChart(e) {
    var mc = _mc(); if (!mc) return false;
    var r = mc.getBoundingClientRect();
    return e.clientX >= r.left && e.clientX <= r.right && e.clientY >= r.top && e.clientY <= r.bottom;
  }

  function _handleChartAction(clientX, clientY) {
    if (_dragging) return;
    var price = _priceAtY(clientY);
    ...
    if (_activeTool === 'eraser') {
      if (!price) return;
      var cl = null, cd = Infinity, pp = price * 0.005;
      _lines.forEach(function(l) {
        var d;
        if (l.type === 'hline') d = Math.abs(l.price - price);
        if (l.type === 'tline') d = Math.min(Math.abs(l.p1.price - price), Math.abs(l.p2.price - price));
  ...
  function _updateBtnStates() {
    ['dt-hline','dt-tline','dt-eraser'].forEach(function(id) {
      var b = document.getElementById(id); if (b) b.classList.toggle('on', _activeTool === id.replace('dt-',''));
    });
  }
  function _clearAll() { _lines.slice().forEach(function(l) { _removeLine(l.id); }); _toast('All cleared'); }
  function _toggleVisibility() {
    ...
    _lines.forEach(function(l) {
      ...
      l.handles.forEach(function(h) { h.style.display = (_visible && l.selected) ? 'block' : 'none'; });
  ...
  window.drawToolActivate = _activate;
  window.drawToolClearAll = _clearAll;
  window.drawToolToggleVis = _toggleVisibility;
