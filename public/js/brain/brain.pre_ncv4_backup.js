...
function renderCircuitBrain() {
  ...
  try {
    ...
    // Update score arc — r=52, circumf = 2π×52 ≈ 327
  ...
  // ── 7. Sync state class + mesh/arc colours ──
  try {
    const state = (typeof BRAIN !== 'undefined' && BRAIN.state) ? BRAIN.state : 'scanning';
    // Set container state class for CSS mesh/anchor/wave reactivity
    const viz = document.getElementById('brainViz');
    if (viz) viz.className = 'nc-container nc-state-' + state;
    const arcCol = state === 'ready' || state === 'armed' ? '#39ff1488'
      : state === 'trading' ? '#39ff14aa'
        : state === 'protect' || state === 'blocked' ? '#ff335588'
          : '#00b8d466';
    document.querySelectorAll('#brainSvg .nc-arc').forEach(t => {
      t.style.stroke = arcCol;
    });
    // Core pulse ring colour
    const core = cbn('zncCore');
    if (core) core.style.stroke = arcCol;
  } catch (_) { }
  ...
  try {
    ...
    // Flow
    const flowVal = (typeof BM !== 'undefined' && BM.flow != null) ? (typeof BM.flow === 'number' ? BM.flow.toFixed(1) : String(BM.flow)) : '—';
