...
function renderCircuitBrain() {
  ...
  try {
    ...
    const branchCol = state === 'ready' || state === 'armed' ? '#39ff1488'
      : state === 'trading' ? '#fbf17588'
        : state === 'protect' || state === 'blocked' ? '#ff335588'
          : '#da643c66';
    document.querySelectorAll('#brainSvg .nc-branch').forEach(t => {
      t.style.stroke = branchCol;
    });
    // Core pulse ring colour
    const core = cbn('zncCore');
    if (core) core.style.stroke = branchCol;
  } catch (_) { }
