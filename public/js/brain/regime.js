...
const RegimeEngine = (function () {
    ...
    function _breakoutStrength(klines) {
        ...
        // Volume ratio
        const volRecent = last5.reduce(function (s, k) { return s + (k.volume || 0); }, 0) / 5;
        const volOld = prev5.reduce(function (s, k) { return s + (k.volume || 0); }, 0) / 5;
        const volScore = volOld > 0 ? Math.min(40, Math.round((volRecent / volOld - 1) * 80)) : 0;
        // ATR expansion (range expansion)
        var rangeRecent = last5.reduce(function (s, k) { return s + (k.high - k.low); }, 0) / 5;
        var rangeOld = prev5.reduce(function (s, k) { return s + (k.high - k.low); }, 0) / 5;
        ...
        var dir = klines[klines.length - 1].close > klines[klines.length - 1].open ? 1 : -1;
        var ftCount = 0;
        for (var i = klines.length - 3; i < klines.length; i++) {
            if (i < 0) continue;
            var dC = klines[i].close > klines[i].open ? 1 : -1;
            if (dC === dir) ftCount++;
        }
        var ftScore = Math.round((ftCount / 3) * 30);
    ...
    function analyzeMarketRegime() {
        try {
            ...
            var klines = (typeof S !== 'undefined' && S.klines) ? S.klines : [];
            ...
            var enhanced = { regime: 'unknown', adx: 0, volMode: '—', structure: '—', squeeze: false, atrPct: 0, slope20: 0 };
            if (typeof detectRegimeEnhanced === 'function') {
                enhanced = detectRegimeEnhanced(klines) || enhanced;
            }

            // ═══ 2. REUSE: BM.volRegime (config.js updateVolRegime) ═══
            var volRegime = (typeof BM !== 'undefined' && BM.volRegime) ? BM.volRegime : '—';

            // ═══ 3. REUSE: BM.sweep (brain.js detectSweepDisplacement) ═
            var sweep = (typeof BM !== 'undefined' && BM.sweep) ? BM.sweep : { type: 'none' };

            // ═══ 4. REUSE: OF.trap (orderflow.js MM Trap Detector) ════
            // [PATCH P2-3] null guard: typeof null !== 'undefined' is true → null.trap throws
            var ofTrap = (typeof OF !== 'undefined' && OF !== null && OF.trap) ? OF.trap : { active: false };

            // ═══ 5. REUSE: OF.sweep / OF.cascade (orderflow.js) ═══════
            // [PATCH P2-3] Same null guard for OF.sweep and OF.cascade
            var ofSweep = (typeof OF !== 'undefined' && OF !== null && OF.sweep) ? OF.sweep : { active: false };
            var ofCascade = (typeof OF !== 'undefined' && OF !== null && OF.cascade) ? OF.cascade : { state: 'idle' };

            // ═══ 6. REUSE: _fakeout (brain.js checkAntiFakeout) ═══════
            var fakeoutInvalid = (typeof _fakeout !== 'undefined') ? !!_fakeout.invalid : false;

            // ═══ 7. REUSE: BRAIN.regimeConfidence (brain.js detectMarketRegime) ═
            var brainConf = (typeof BRAIN !== 'undefined' && BRAIN.regimeConfidence) ? BRAIN.regimeConfidence : 0;

            // ═══ 8. NEW: compute wick chaos ════════════════════════════
            var wickChaos = _wickChaos(klines, 10);

            // ═══ 9. NEW: breakout strength ═════════════════════════════
            var brkStrength = _breakoutStrength(klines);
            ...
            var volStateMap = { 'LOW': 'low', 'MED': 'normal', 'HIGH': 'high', 'EXTREME': 'extreme' };
            var volatilityState = volStateMap[volRegime] || 'normal';
            ...
            // Active OF sweep with cascade, OR panic regime with active sweep displacement
            ...
            if (_prevRegime !== null && _prevRegime !== regime) {
                if (typeof ZLOG !== 'undefined') {
                    ZLOG.push('INFO', '[RE] ' + _prevRegime + '\u2192' + regime + ' conf=' + confidence + '%',
            ...
            if (typeof console !== 'undefined') console.warn('[RE] analyzeMarketRegime error:', e.message || e);
            return Object.assign({}, _DEFAULT);
        }
    }
    ...
    function getState() {
        if (typeof BM !== 'undefined' && BM.regimeEngine) return BM.regimeEngine;
        return Object.assign({}, _DEFAULT);
    }
