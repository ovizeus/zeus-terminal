// Zeus v122 — brain/phaseFilter.js
...
const PhaseFilter = (function () {
    ...
    function getAllowedSetups(phase, trendBias) {
        switch (phase) {
            case 'TREND':
    ...
    function getBlockedSetups(phase, trendBias) {
        switch (phase) {
            case 'TREND':
    ...
    // ══════════════════════════════════════════════════════════════════
    function evaluateMarketPhase(input) {
        try {
            ...
            var trendBias = input.trendBias || 'neutral';
            var volatilityState = input.volatilityState || 'normal';
            var trapRisk = input.trapRisk || 0;

            // ── 1. Map regime → phase ──
            var phase = mapRegimeToPhase(regime);

            // ── 2. Determine setups ──
            var allowedSetups = getAllowedSetups(phase, trendBias);
            var blockedSetups = getBlockedSetups(phase, trendBias);

            // ── 3. Determine risk ──
            var riskMode = calcRiskMode(phase, volatilityState, trapRisk, confidence);
            var sizeMultiplier = calcSizeMultiplier(phase, riskMode, trapRisk, confidence);
            ...
            if (_prevPhase !== null && _prevPhase !== phase) {
                if (typeof ZLOG !== 'undefined') {
                    ZLOG.push('INFO', '[PF] ' + _prevPhase + '\u2192' + phase + ' risk=' + riskMode + ' size=' + sizeMultiplier.toFixed(2),
            ...
            if (typeof console !== 'undefined') console.warn('[PF] evaluateMarketPhase error:', e.message || e);
            return Object.assign({}, _DEFAULT);
        }
    }
    ...
    function getState() {
        if (typeof BM !== 'undefined' && BM.phaseFilter) return BM.phaseFilter;
        return Object.assign({}, _DEFAULT);
    }
