...
// ════════════════════════════════════════════════════════════════
// ALIEN UPGRADE BAY — AUB v1.0
// All 8 modules + UI control
// ════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════
// [MOVED TO TOP] AUB_COMPAT

function aubCheckCompat() {
  const proto = location.protocol;
  const isSecure = proto === 'https:';
  const isLocal = proto === 'file:' || proto === 'blob:' || proto === 'content:';

  AUB_COMPAT.ws = typeof WebSocket !== 'undefined';
  AUB_COMPAT.audio = typeof (window.AudioContext || window.webkitAudioContext) !== 'undefined';
  AUB_COMPAT.crypto = !!(window.crypto && window.crypto.subtle);
  AUB_COMPAT.swDisabled = isLocal || !isSecure;
  AUB_COMPAT.sw = !AUB_COMPAT.swDisabled && ('serviceWorker' in navigator);

  // Disable ServiceWorker if non-https
  if (AUB_COMPAT.swDisabled) {
    // SW already guarded in registerServiceWorker() — just mark here
    AUB_COMPAT.sw = false;
  }

  const allOk = AUB_COMPAT.ws && AUB_COMPAT.audio && AUB_COMPAT.crypto;
...
// ════════════════════════════════════════════════════════════════
function _aubGuard(name, val, test) {
  AUB.guardCount++;
  const ok = test(val);
  if (!ok) {
...
function _aubWrapPublicFunctions() {
  // setSymbol guard
  const _origSetSymbol = window.setSymbol;
  if (typeof _origSetSymbol === 'function') {
    window.setSymbol = function (sym) {
      if (!_aubGuard('setSymbol', sym, v => typeof v === 'string' && v.length >= 3 && v.length <= 20)) return;
  ...
  const _origToggleAT = window.toggleAutoTrade;
  if (typeof _origToggleAT === 'function') {
    window.toggleAutoTrade = function () {
      if (typeof AT === 'undefined') { console.warn('[AUB GUARD] AT not initialized'); return; }
      AUB.guardCount++;
      _aubUpdateGuardUI();
...
// ════════════════════════════════════════════════════════════════
// [MOVED TO TOP] AUB_PERF

function _aubRafTick(ts) {
...
function aubBBSnapshot(event, extra) {
  try {
    const snap = {
      ...
      ts: Date.now(),
      event: event || 'unknown',
      score: (typeof BM !== 'undefined' ? BM.entryScore : null) || 0,
      regime: (typeof BRAIN !== 'undefined' ? BRAIN.regime : null) || '—',
      session: new Date().getUTCHours(),
      mode: (typeof S !== 'undefined' ? S.mode : null) || '—',
      blockReason: (typeof BlockReason !== 'undefined' ? BlockReason.text() : '—'),
    };
    AUB.bb.unshift(snap);
...
function _aubUpdateBBCard() {
  AUB_PERF.setDOM('aub-bb-count', 'Snapshots: ' + AUB.bb.length);
  const last = AUB.bb[0];
  AUB_PERF.setDOM('aub-bb-last', last
    ? 'Last: ' + last.event + ' @' + new Date(last.ts).toTimeString().slice(0, 8)
    : 'Last: —');
}

function aubBBExport() {
  try {
...
// ════════════════════════════════════════════════════════════════
function aubCalcMTFStrength() {
  try {
    const rsi = (typeof S !== 'undefined' && S.rsi) ? S.rsi : {};
    const klines = (typeof S !== 'undefined' && S.klines) ? S.klines : [];
    // ADX from klines (already calculated in brain)
    const adx = (typeof BRAIN !== 'undefined' && BRAIN.regime) ? 1 : 0;
...
function _aubUpdateMTFCard(s5m, s15m, s1h, s4h, penalty) {
  const pct = v => Math.round(v * 100) + '%';
  // [B24 FIX] Removed broken set() calls with wrong parameter order
  const ids = [['aub-mtf-4h', s4h, 'aub-mtf-4h-v'], ['aub-mtf-1h', s1h, 'aub-mtf-1h-v'], ['aub-mtf-15m', s15m, 'aub-mtf-15m-v'], ['aub-mtf-5m', s5m, 'aub-mtf-5m-v']];
  ids.forEach(([bid, val, vid]) => {
...
// Passive: called by aubRefreshAll on expand — no own interval (req 9)
// ════════════════════════════════════════════════════════════════
function aubCalcCorrelation() {
  try {
    // Use wlPrices history if available, else skip
    // Simple: compare % change direction over last N ticks
    const btcPrice = (typeof S !== 'undefined') ? S.price : 0;
    if (!btcPrice) return;

    // We use watchlist data snapshots if available
    const eth = (typeof wlPrices !== 'undefined' && wlPrices['ETHUSDT']) ? wlPrices['ETHUSDT'] : null;
    const sol = (typeof wlPrices !== 'undefined' && wlPrices['SOLUSDT']) ? wlPrices['SOLUSDT'] : null;
    ...
    const sym = (typeof S !== 'undefined' && S.symbol) ? S.symbol : '';
...
function _aubUpdateCorrCard() {
  const fmt = v => v !== null ? (v > 0 ? '+' : '') + v.toFixed(2) : '—';
  AUB_PERF.setDOM('aub-corr-eth', fmt(AUB.corr.eth));
...
// ════════════════════════════════════════════════════════════════
function aubMacroImport() { document.getElementById('aub-macro-file')?.click(); }
function aubMacroClear() {
  AUB.macroEvents = [];
...
function aubGetActiveMacroRisk() {
  const now = Date.now();
...
function _aubRenderMacroEvents() {
  ...
  const now = Date.now();
  el_m.innerHTML = AUB.macroEvents.slice(0, 5).map(ev => {
    const diff = ev.ts - now;
...
// ════════════════════════════════════════════════════════════════
// [MOVED TO TOP] AUB_SIM_KEY

function aubSimRun() {
  if (AUB.simRunning) return;
  AUB.simRunning = true;
  AUB_PERF.setDOM('aub-sim-status', 'Status: Running...');
  // Async so UI doesn't freeze
  setTimeout(_aubSimWorker, 100);
}

function _aubSimWorker() {
  try {
    const klines = (typeof S !== 'undefined' && S.klines) ? S.klines : [];
    if (klines.length < 50) {
      AUB_PERF.setDOM('aub-sim-status', 'Status: Need 50+ bars');
    ...
    let best = { sl: 1.0, rr: 2.0, score: 0, wins: 0, total: 0 };
    ...
    _safeLocalStorageSet(AUB_SIM_KEY, { best, ts }); // FIX 22

    AUB_PERF.setDOM('aub-sim-status', 'Status: Done (' + ts + ')');
    AUB_PERF.setDOM('aub-sim-last', 'Last run: ' + ts);
    ...
    AUB_PERF.setDOM('aub-sim-status', 'Status: Error — ' + (e.message || '?'));
  }
  AUB.simRunning = false;
}

function aubSimApply() {
  ...
  const slInput = document.getElementById('atSL');
...
function _aubLoadSim() {
  try {
    const raw = localStorage.getItem(AUB_SIM_KEY);
    if (raw) {
      const data = JSON.parse(raw);
      AUB.simResult = data.best;
      AUB_PERF.setDOM('aub-sim-last', 'Last run: ' + (data.ts || '—'));
    }
  } catch (_) { }
}

// Nightly auto-run (once per day, at init)
function _aubCheckNightlySim() {
  try {
    const raw = localStorage.getItem(AUB_SIM_KEY);
...
function _aubUpdateDataBadge() {
  ...
  const hasPrice = (typeof S !== 'undefined') && S.price > 0;
...
function initAUB() {
  ...
  if (el_aub) {
    ...
    AUB.expanded = (saved === '1') && window.innerWidth >= 600;
    el_aub.className = AUB.expanded ? 'expanded' : 'collapsed';
    const txt = document.getElementById('aub-toggle-txt');
    if (txt) txt.textContent = AUB.expanded ? 'COLLAPSE' : 'EXPAND';
  }
  ...
  Intervals.set('aubRefresh', _aubUpdateDataBadge, 3000);

  // Subscribe to zeusReady event for nightly sim (req 2, 9)
  window.addEventListener('zeusReady', () => {
