...
let mockOptedIn = false;
jest.mock('../../server/services/ml/mlLiveOptin', () => ({
    isOptedIn: jest.fn(() => mockOptedIn),
}));
...
describe('influenceEligibility REAL opt-in gate', () => {
    beforeEach(() => { mockOptedIn = false; mockFlags.ML_LIVE_OPTIN_REQUIRED = true; });
    ...
    test('REAL + no opt-in → ineligible with live_optin_missing', () => {
        ...
        expect(mlLiveOptin.isOptedIn).toHaveBeenCalledWith(1);
    });
    ...
    test('DEMO does not require opt-in', () => {
        const r = checkEligibility({ ...base, env: 'DEMO' });
        expect(r.reason).toBe('insufficient_observations');
    });
    ...
    test('flag missing/undefined → still gated (fail-closed polarity)', () => {
        mockFlags.ML_LIVE_OPTIN_REQUIRED = undefined;
        const r = checkEligibility({ ...base, env: 'REAL' });
        expect(r.reason).toBe('live_optin_missing');
    });
