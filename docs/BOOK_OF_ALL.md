...
13. **ML pre-REAL refinements (din `docs/ML_SOAK_ACCEPTANCE.md`)** — sub-task-uri ÎNAINTE de Stage 5 REAL (parte din gardul flip-ului, vezi item 2): DD2 tests dedicate, rafinare digest-lookup Stage 5, cablare `evaluatePerformance` cron, script soak-validation automat, endpoint `/api/ml/stage-promote` (sign-off + audit), cablare trigger auto-halt drawdown >10%/24h. + CSP 4-faze (`docs/CSP-MIGRATION-PLAN.md`, leagă de bug securitate #3).
