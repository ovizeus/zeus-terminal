...
12. **Vault — testează-l TU înainte de parole reale** — seiful zero-knowledge e livrat (b236) + cripto dovedit deterministic (7 teste), DAR n-am putut completa testul UI automat live (Playwright a dat rateuri). *De făcut o dată:* creează seif cu o parolă de TEST → pune un secret de test → încuie/descuie → confirmă că se descifrează înapoi. Abia după aia pune parole reale. (Dacă uiți parola = pierdut definitiv, zero-knowledge.)
